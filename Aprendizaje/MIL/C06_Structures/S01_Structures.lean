...
-- "protected" obliga a que el nombre del teorema sea Point.add_comm incluso aunque esté abierto el espacio "Point"
...
def weightedAverage (lambda : Real) (lambda_nonneg : 0 ≤ lambda) (lambda_le : lambda ≤ 1)
  ...
  x_nonneg := by
    ...
    · apply mul_nonneg _ b.x_nonneg
      linarith
  y_nonneg := by
    apply add_nonneg
    · apply mul_nonneg lambda_nonneg a.y_nonneg
    · apply mul_nonneg _ b.y_nonneg
      linarith
  z_nonneg := by
    apply add_nonneg
    · apply mul_nonneg lambda_nonneg a.z_nonneg
    · apply mul_nonneg _ b.z_nonneg
      linarith
  sum_eq := by
...
def weightedAverage (n: ℕ)(lambda : Real) (lambda_nonneg : 0 ≤ lambda) (lambda_le : lambda ≤ 1)
    ...
    sum_eq_one := by
      dsimp
      simp [Finset.sum_add_distrib, mul_comm lambda _, mul_comm (1-lambda) _,  ← Finset.sum_mul,
