...
private theorem aux {α : Type*} [LinearOrderedRing α] {x y : α} (h : x ^ 2 + y ^ 2 = 0) : x = 0 := by
  have h' : x^2 = 0 := by
...
theorem norm_nonneg (x : gaussInt) : 0 ≤ norm x := by
  rw [norm]
  apply add_nonneg (sq_nonneg x.re) (sq_nonneg x.im)
...
theorem natAbs_norm_mod_lt (x y : gaussInt) (hy : y ≠ 0) :
  ...
  apply Int.ofNat_lt.mp -- La flecha hacia arriba significa que los ve como enteros (creo)
...
/- No me gusta esta forma de hacerlo
...
/- Otra forma de hacerlo  (que me gusta más) donde no tenemos que especificar que sea una anillo
   ni que no es trivial, porque ya hemos declaro esos "instance" previamente -/
