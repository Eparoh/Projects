...
/- Es en esencia lo mismo que arriba, pero sin necesidad de recordar el orden
   de la conjunción -/
...
#check MonoidHom₁.ext
#check MonoidHom₁.ext_iff

example [Monoid G] [Monoid H] (f : MonoidHom₁ G H) : f.toFun 1 = 1 := f.map_one
...
class MonoidHomClass₃ (F : Type) (M N : outParam Type) [Monoid M] [Monoid N] extends DFunLike F M (fun _ ↦ N) where
  map_one : ∀ f : F, f 1 = 1
...
instance (α β : Type) [LE α] [Monoid α] [LE β] [Monoid β] : OrderPresHomClass (OrderPresMonoidHom α β) α β where
