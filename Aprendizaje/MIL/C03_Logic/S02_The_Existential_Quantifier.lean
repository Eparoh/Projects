...
-- Teoremas para lo siguiente

theorem fnLb_add {f g : ℝ → ℝ} {a b : ℝ} (hfa : FnLb f a) (hgb : FnLb g b) :
    FnLb (fun x ↦ f x + g x) (a + b) :=
  fun x ↦ add_le_add (hfa x) (hgb x)

theorem fnUb_mul_left {f: ℝ → ℝ} {a c : ℝ} (hfa: FnUb f a) (h : 0 ≤ c) : FnUb (fun x ↦ c * f x) (c * a) := by
  intro x
  dsimp
  apply mul_le_mul_of_nonneg_left (hfa x) h
...
example (surjg : Surjective g) (surjf : Surjective f) : Surjective fun x ↦ g (f x) := by
  intro x
  dsimp
  rcases surjg x with ⟨a, gxa⟩
  rcases surjf a with ⟨b, fba⟩
  use b
  rw [fba, gxa]
