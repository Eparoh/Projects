...
/- Coercion to see a net as a function -/

instance {X : Type*} : CoeFun (Net X) (fun s ↦ s.D → X) where
    coe := Net.net

attribute [coe] Net.net
...
def Net.mk' {X D: Type u} (h: DirectedSet D) (s: D → X) : Net X :=
{
  D := D
  directedSet := h
  net := s
...
@[simps]
instance Net.instSequences {X: Type} (s: ℕ → X) : Net X := Net.mk' (inferInstance) s
...
@[simps]
instance LE.instProduct {α β : Type u} [LE α] [LE β] : LE (α × β) where
  le := fun (a, b) ↦ (fun (a', b') ↦ a ≤ a' ∧ b ≤ b')

instance Preorder.instProduct {α β : Type u} [Preorder α] [Preorder β] : Preorder (α × β) where
  toLE := LE.instProduct
...
instance DirectedSet.instProduct (D E : Type u) [DirectedSet D] [DirectedSet E] : DirectedSet (D × E) where
...
@[simps]
instance Net.instProduct {X D E: Type u} (hD : DirectedSet D) (hE : DirectedSet E) (s: D × E → X) : Net X :=
...
@[simps]
def Net.function {X Y: Type u} (s : Net X) (f: X → Y) : Net Y := Net.mk' (s.directedSet) (fun (d: s.D) ↦ f (s d))
...
theorem t2_iff_net_unique_limit {X : Type*} [TopologicalSpace X] :
  T2Space X ↔ ∀ (s : Net X) (x y : X), (Limit s x ∧ Limit s y) → x = y := by
    constructor
    ...
    · intro h
      ...
      have slimit : Limit s x ∧ Limit s y := by
        constructor
        · dsimp [Limit]
          intro U U_nhds
...
theorem accumulationPoint_iff_exists_subnet {X: Type*} [TopologicalSpace X] (x: X) (s: Net X) :
  AccumulationPoint s x ↔ ∃ (s': Net X), (Subnet s s' ∧ Limit s' x) := by
    constructor
    · intro h
      dsimp [AccumulationPoint] at h
      have existence : ∀ V ∈ 𝓝 x, ∀ (d: s.D), ∃ (e: s.D), (d ≤ e ∧ s e ∈ V) := by
        intro V V_nhds d
        exact h d V V_nhds
      ...
      let i : {V | V ∈ 𝓝 x} × s.D → s.D := fun (⟨V, V_in⟩, d)  ↦  if h: ∃ (e: s.D), (d ≤ e ∧ s e ∈ V) then Classical.choose h else d
      ...
      · dsimp [Subnet]
        ...
        · intro d
          ...
          intro e h'
          dsimp [s'] at e
          simp [s', DirectedSet.instNeighbourhoodLeft_le x] at h'
          dsimp [i]
...
theorem mem_closure_iff_exists_net {X: Type*} [TopologicalSpace X] (A: Set X) (x : X):
  x ∈ closure A ↔ ∃ (s: Net X), (∀ (d: s.D), s d ∈ A) ∧ Limit s x := by
    constructor
    · intro h
...
theorem isClosed_iff_Limit_self {X: Type*} [TopologicalSpace X] (C: Set X) :
  IsClosed C ↔ ∀ (x : X), ∀ (s : Net X), (∀ (d: s.D), s d ∈ C) → Limit s x → x ∈ C := by
    constructor
...
K = ∅ ∨ ∀ (s : Net X), (∀ (d : s.D), s d ∈ K) → (∃ (s': Net X), ∃ x ∈ K, Subnet s s' ∧ Limit s' x) := by
  constructor
  · intro K_Compact
    ...
    let A : s.D → Set X := fun c ↦ closure {x | ∃ (d : s.D), c ≤ d ∧ x = s d}
...
/- Characterization of continuity by nets:
    f is continuous in x iff for every net s : D → X convergent to x, f (s x) converges to f x -/
theorem continuous_iff_image_of_net_converges {X Y : Type u} [TopologicalSpace X] [TopologicalSpace Y] (f: X → Y) (x : X):
